...
/**
 * From Chrome extension `Synology Download Station`
 */
export function SYNOFC() {
    SYNO.Encryption = {};
    ...
    SYNO.Encryption.EncryptParam = function (e, cipherKey, rsaModulus, cipherToken, timeBias) {
        ...
        for (var key in e) {
            array.push(encodeURIComponent(key) + "=" + encodeURIComponent(e[key]));
        }

        var encodedData = array.join("&").replace(/\+/g, '%20');
